...
class RecordPurchaseOrderLog(tk.Toplevel, RecordBase):
    def __init__(self, container, mysql, user, purchase_order_id, *args, **kwargs):
        ...
        RecordBase.__init__(self, mysql, user, POPUP_TITLE="Purchase Order Log's Description", button_2_text=None) # Class.contructor(instance or object) is right syntax
    ...
    def renderLabels(self, record_table_name):
        ...
        lb_4 = CLabel(self.frame, text="Purchase Order Id: ", font=SINGLE_RECORD_DISPLAY_LABEL_TITLE_FONT)
        lb_4.grid(row=4, column=0, sticky="nsew")
        
        self.purchase_order_id_label = CLabel(self.frame, font=SINGLE_RECORD_DISPLAY_LABEL_VALUE_FONT)
        self.purchase_order_id_label.grid(row=4, column=1, sticky="nsew")
    ...
    def fetchDataAndRender(self):
        # Fetch Data
        data = self.queryFetchSinglePurchaseOrderLog(self.purchase_order_id)
        # Display Data
        self.log_label.configure(text=data[0][PURCHASE_ORDER_LOGS_LOG])
        ...
        self.purchase_order_id_label.configure(text=data[0][PURCHASE_ORDER_LOGS_FOREIGNKEY_PURCHASE_ORDER_ID])
        self.username_label.configure(text=data[0][USERNAME])
        self.username_id_label.configure(text=data[0][PURCHASE_ORDER_LOGS_FOREIGNKEY_USER_ID])
