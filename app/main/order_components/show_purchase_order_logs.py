...
from app.main.order_components.record_purchase_order_logs import RecordPurchaseOrderLog

class ShowPurchaseOrderLogs(tk.Toplevel, TreeEssentials):
    
    def __init__(self, container, mysql, user, purchase_order_id, page_heading="Purchase Order Logs:"):
       ...
       # Get Product Table Logs with required columns with foreign key relations values
        self.saved_query_purchase_order_log_morphed = f"""
        SELECT * FROM 
        (SELECT A.{PURCHASE_ORDER_LOGS_ID}, A.{PURCHASE_ORDER_LOGS_LOG}, A.{PURCHASE_ORDER_LOGS_FOREIGNKEY_PURCHASE_ORDER_ID},
        ...
        WHERE A.{PURCHASE_ORDER_LOGS_FOREIGNKEY_PURCHASE_ORDER_ID} = {self.purchase_order_id})
        AS merge_table"""
        ...
        self.defineSearchButtons(
            ...
            refreshedTableMethod=lambda: self.definingRowsOfParticularTree(
                dataFetchingMethod=lambda: self.queryFetchPaginatedLogs(
                    self.offset,
                    self.limit,
                    query_to_get=self.saved_query_purchase_order_log_morphed
                    )
                ),
            searchedRefreshTableMethod=lambda: self.definingRowsOfParticularTree(
                ...
                selectable_columns_from_table_query=self.saved_query_purchase_order_log_morphed,
                table_name=PURCHASE_ORDER_LOGS_LOG,
        ...
        self.definingEventBindings(
            ...
            RecordPopupClass=lambda: RecordPurchaseOrderLog(
                container=self,
                mysql=self.mysql,
    ...
    def definingRowsOfParticularTree(self, dataFetchingMethod):
        ...
        if len(data) >= 1:
            ...
            for i in range(len(data)):
                ...
                log = self.morphText(data[i][PURCHASE_ORDER_LOGS_LOG], 144)        
    ...
    def createTreeAndConfiguration(self):
        ...
        columnTuple = ("DATE", "TIME", PURCHASE_ORDER_LOGS_LOG, USERNAME)
        ...
        self.tree.heading(PURCHASE_ORDER_LOGS_LOG, text="Logs")
        ...
        self.tree.column(PURCHASE_ORDER_LOGS_LOG, anchor="center", stretch=tk.YES, width=100)
