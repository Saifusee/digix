...
from app.main.order_components.record_sales_order_logs import RecordSalesOrderLog

class ShowSalesOrderLogs(tk.Toplevel, TreeEssentials):
    
    def __init__(self, container, mysql, user, sales_order_id, page_heading="Sales Order Logs:"):
       ...
       # Get Product Table Logs with required columns with foreign key relations values
        self.saved_query_sales_order_log_morphed = f"""
        SELECT * FROM 
        (SELECT A.{SALES_ORDER_LOGS_ID}, A.{SALES_ORDER_LOGS_LOG}, A.{SALES_ORDER_LOGS_FOREIGNKEY_SALES_ORDER_ID},
        ...
        WHERE A.{SALES_ORDER_LOGS_FOREIGNKEY_SALES_ORDER_ID} = {self.sales_order_id})
        AS merge_table"""
        ...
        self.defineSearchButtons(
            ...
            refreshedTableMethod=lambda: self.definingRowsOfParticularTree(
                dataFetchingMethod=lambda: self.queryFetchPaginatedLogs(
                    self.offset,
                    self.limit,
                    query_to_get=self.saved_query_sales_order_log_morphed
                    )
                ),
            searchedRefreshTableMethod=lambda: self.definingRowsOfParticularTree(
                ...
                selectable_columns_from_table_query=self.saved_query_sales_order_log_morphed,
                table_name=SALES_ORDER_LOGS_LOG,
        ...
        self.definingEventBindings(
            ...
            RecordPopupClass=lambda: RecordSalesOrderLog(
                container=self,
                mysql=self.mysql,
    ...
    def definingRowsOfParticularTree(self, dataFetchingMethod):
        ...
        if len(data) >= 1:
            ...
            for i in range(len(data)):
                ...
                log = self.morphText(data[i][SALES_ORDER_LOGS_LOG], 144)        
    ...
    def createTreeAndConfiguration(self):
        ...
        columnTuple = ("DATE", "TIME", SALES_ORDER_LOGS_LOG, USERNAME)
        ...
        self.tree.heading(SALES_ORDER_LOGS_LOG, text="Logs")
        ...
        self.tree.column(SALES_ORDER_LOGS_LOG, anchor="center", stretch=tk.YES, width=100)
