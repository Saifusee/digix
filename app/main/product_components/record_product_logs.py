...
class RecordProductLog(tk.Toplevel, RecordBase):
    def __init__(self, container, mysql, user, product_log_id, *args, **kwargs):
        ...
        RecordBase.__init__(self, mysql, user, POPUP_TITLE="Product Log's Description", button_2_text=None) # Class.contructor(instance or object) is right syntax
        
        # necessary instance variable for particular class
        self.product_log_id = product_log_id
    ...
    def renderLabels(self, record_table_name):
        ...
        lb_4 = CLabel(self.frame, text="Product Name: ", font=SINGLE_RECORD_DISPLAY_LABEL_TITLE_FONT)
        lb_4.grid(row=4, column=0, sticky="nsew")
        
        self.product_label = CLabel(self.frame, font=SINGLE_RECORD_DISPLAY_LABEL_VALUE_FONT)
        self.product_label.grid(row=4, column=1, sticky="nsew")
        
        lb_5 = CLabel(self.frame, text="Product ID: ", font=SINGLE_RECORD_DISPLAY_LABEL_TITLE_FONT)
        lb_5.grid(row=5, column=0, sticky="nsew")
        
        self.product_id_label = CLabel(self.frame, font=SINGLE_RECORD_DISPLAY_LABEL_VALUE_FONT)
        self.product_id_label.grid(row=5, column=1, sticky="nsew")
    ...
    def fetchDataAndRender(self):
        # Fetch Data
        data = self.queryFetchSingleProductLog(self.product_log_id)
        # Display Data
        self.log_label.configure(text=data[0][PRODUCT_LOGS_LOG])
        ...
        self.product_label.configure(text=data[0][PRODUCT_NAME])
        self.product_id_label.configure(text=data[0][PRODUCT_LOGS_FOREIGNKEY_PRODUCT_ID])
        self.username_label.configure(text=data[0][USERNAME])
        self.username_id_label.configure(text=data[0][PRODUCT_LOGS_FOREIGNKEY_USER_ID])
