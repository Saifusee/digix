...
class RecordUserLog(tk.Toplevel, RecordBase):
    def __init__(self, container, mysql, user, user_log_id, *args, **kwargs):
        ...
        RecordBase.__init__(self, mysql, user, POPUP_TITLE="User Log's Description", button_2_text=None) # Class.contructor(instance or object) is right syntax
    ...
    def renderLabels(self, record_table_name):
        ...
        lb_4 = CLabel(self.frame, text="Target User (username): ", font=SINGLE_RECORD_DISPLAY_LABEL_TITLE_FONT)
        lb_4.grid(row=4, column=0, sticky="nsew")
        
        self.target_username_label = CLabel(self.frame, font=SINGLE_RECORD_DISPLAY_LABEL_VALUE_FONT)
        self.target_username_label.grid(row=4, column=1, sticky="nsew")
        
        lb_5 = CLabel(self.frame, text="Target User (user id): ", font=SINGLE_RECORD_DISPLAY_LABEL_TITLE_FONT)
        lb_5.grid(row=5, column=0, sticky="nsew")
        
        self.target_user_id_label = CLabel(self.frame, font=SINGLE_RECORD_DISPLAY_LABEL_VALUE_FONT)
        self.target_user_id_label.grid(row=5, column=1, sticky="nsew")
        
        lb_6 = CLabel(self.frame, text="Activity done by (username): ", font=SINGLE_RECORD_DISPLAY_LABEL_TITLE_FONT)
        ...
        lb_7 = CLabel(self.frame, text="Activity done by (user id):: ", font=SINGLE_RECORD_DISPLAY_LABEL_TITLE_FONT)
    ...
    def fetchDataAndRender(self):
        # Fetch Data
        data = self.queryFetchSingleUserLog(self.user_log_id)
        # Display Data
        self.log_label.configure(text=data[0][USER_LOGS_LOG])
        ...
        self.target_username_label.configure(text=data[0][TARGET_USER])
        self.target_user_id_label.configure(text=data[0][USER_LOGS_FOREIGNKEY_TARGET_USER_ID])
        self.username_label.configure(text=data[0][ACTIVE_USER])
        self.username_id_label.configure(text=data[0][USER_LOGS_FOREIGNKEY_ACTIVE_USER_ID])
