import sys
from cusimp.constants import ORGANIZATION_NAME
sys.path.append("..")
...
import cusimp.table_constants as table_constant
...
class Login(ttk.Frame, AuthenticateBase):
    def __init__(self, container, mysql, raise_register_frame, window_instance, *args, **kwargs):
        ...
        #PUTTING BUTTONS
        self.put_buttons(bottom_frame, raise_register_frame, window_instance)
    ...
    def put_labels_and_fields(self, frame):
        ...
        self.password_entry.grid(row=1, column=1, padx=(10,10), pady=(10,10))
        ...
        self.new_password_entry = ttk.Entry(frame, width=40, textvariable=self.new_password_value, font=('TkDefaultFont', 10))
    ...
    def put_buttons(self, frame, raise_register_frame, window_instance):
        #SUBMIT BUTTON
        self.submit_button = ttk.Button(frame, text="Submit", command=lambda: [self.submit_function(window_instance)], style="LoginRegisterButton.TButton")
    ...
    def forgot_password(self):
        if (self.username_or_email_value.get() == ""):
            ...
            try: 
                self.password_value.set("")
                query1=f"SELECT * FROM {table_constant.USER_TABLE_NAME} WHERE {table_constant.EMAIL} LIKE '{self.username_or_email_value.get()}' OR {table_constant.USERNAME} like '{self.username_or_email_value.get()}'"
                self.mysql.query.execute(query1)
                otp_value = randint(11111111, 99999999)
                self.otp_ref = f"{otp_value}-{self.username_or_email_value.get()}-{otp_value}"
                ...
                if (query_data == None):
                    ...
                    self.password_entry["style"] = "TEntry"
                    self.otp_entry.grid(row=2, column=1, padx=(10,10), pady=(10,10))
                    self.submit_otp_button.grid(row=2, column=0, padx=(10, 10))
                    self.otp_entry.focus()
                    self.dummy_user = query_data #It holds temporary value of email
                    query2 = f"INSERT INTO {table_constant.OTP_TABLE_NAME} ({table_constant.OTP_REFERENCE}, {table_constant.OTP_VALUE}) VALUES ('{self.otp_ref}', {otp_value})"
                    self.mysql.query.execute(query2)
                    self.mysql.db_connection.commit()
                    try:
                        message =f"OTP for password reset = {otp_value}"
                        DigixMail(query_data[table_constant.EMAIL], f"Password Reset: {ORGANIZATION_NAME}", message)
    ...
    def submit_otp(self):
        try:
            query1 = f"SELECT * FROM {table_constant.OTP_TABLE_NAME} WHERE {table_constant.OTP_REFERENCE} LIKE '{self.otp_ref}'"
            self.mysql.query.execute(query1)
            query_data = self.mysql.query.fetchone()
            if(query_data != None):
                self.otp_entry.grid_forget()
                ...
                self.new_password_entry.grid(row=3, column=1, padx=(10,10), pady=(10,10))
                self.new_password_button.grid(row=4, column=0, columnspan=2, sticky="ew")
            else:
                self.errors.set("Invalid OTP")
        except Exception as errors:
            print(f"Development Error (While VewrifyingOTP): {errors}")
            
    def new_password_submit(self):
        try:
            if (self.dummy_user[table_constant.PASSWORD] == self.new_password_value.get()):
                self.errors.set("New Password cannot be same as old password.")
            else:
                try:
                    query1 = f"UPDATE {table_constant.USER_TABLE_NAME} SET {table_constant.PASSWORD} = '{self.new_password_value.get()}' WHERE {table_constant.USER_ID} = {self.dummy_user[table_constant.USER_ID]}"
                    self.mysql.query.execute(query1)
                    self.mysql.db_connection.commit()
                except Exception as errors:
                    print(f"Development Error (UpdatingNewPasswordOnDatabse): {errors}")
                try:
                    query2 = f"DELETE FROM {table_constant.OTP_TABLE_NAME} WHERE {table_constant.OTP_REFERENCE} = '{self.otp_ref}'"
                    self.mysql.query.execute(query2)
                    self.mysql.db_connection.commit()
                except Exception as errors:
                ...
                after_timer = threading.Timer(5, self.reset_field_data)
                after_timer.start()
        except Exception as errors:
            print(f"Development Error: {errors}")                
    
    def validate_email_or_username(self, event):
        username_email_regex = r"\b([A-Za-z_]+[A-Za-z0-9_@.]*){2,50}\b"
        username_value = self.username_or_email_value.get()
        ...
        elif (not re.fullmatch(username_email_regex, username_value)):
            self._validation_fail("Username and email must be between 2-50 characters. Characters other than [A-Z], [a-z], [0-9], period(.) and underscore not allowed(_). Cannot start and end with (.)", *parameter_false_tuple)
        else:
    ...
    def validate_new_password(self, event):
        password_value = self.new_password_value.get()
        password_regex = r"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,15}$"
        if (password_value == ""):
    ...
    def submit_function(self, window_instance):
        try:
            email_value = self.username_or_email_value.get()
            password_value = self.password_value.get()
            self.mysql.query.execute(f"SELECT * FROM {table_constant.USER_TABLE_NAME} WHERE ({table_constant.EMAIL} like '{email_value}') or ({table_constant.USERNAME} like '{email_value}')")
            user = self.mysql.query.fetchone()
            if (user != None): #ensure that dictionary not empty (empty dict return None)
                if (user[table_constant.PASSWORD] == password_value):
                    self.error_label["style"] = "SuccessfulLoginRegisterLabel.TLabel"
                    self.errors.set("Login Successfull") 
                    self.reset_field_data()
                    window_instance.user = user
                    window_instance.destroy()
                else:
                    self.errors.set("Incorrect Password")
                    window_instance.user = dict()
            else:
