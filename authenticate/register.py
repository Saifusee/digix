...
import cusimp.constants as constants
...
class Register(ttk.Frame, AuthenticateBase):
    def __init__(self, container, mysql, raise_login_frame, *args, **kwargs):
    ...
    def validate_email(self, event):
        email_value = self.email_value.get()
    ...
    def validate_username(self, event):
        username_value = self.username_value.get()
    ...
    def submit_only_email(self): # When EMAIL submit for OTP
        try: 
            ...
            otp_gen = randint(11111111, 99999999)
            self.otp_ref = f"{otp_gen}-{self.email_value.get()}-{otp_gen}"
            self.mysql.query.execute(f"""INSERT INTO {table_constant.OTP_TABLE_NAME}
                                ({table_constant.OTP_REFERENCE}, {table_constant.OTP_VALUE}) 
                                VALUE ('{self.otp_ref}', {otp_gen})""")
        ...
        # send email with otp
        try:
            message = f"Welcome to {constants.ORGANIZATION_NAME}, the OTP for E-Mail verification is = {otp_gen}"
            mail.DigixMail(self.email_value.get(), f"E-Mail Verification: {constants.ORGANIZATION_NAME}", message)
    ...
    def submit_only_otp(self): # When OTP submit for EMAIL verification
        try:
            otp_value = int(self.otp_value.get())
            query = f"SELECT * FROM {table_constant.OTP_TABLE_NAME} WHERE {table_constant.OTP_REFERENCE} LIKE '{self.otp_ref}'"
    ...
    def submit_function(self):
        try: 
            ...
            query = f"""INSERT INTO {table_constant.USER_TABLE_NAME}
                    ({table_constant.EMAIL}, {table_constant.USERNAME}, {table_constant.PASSWORD}) 
                    VALUES ('{email_value}', '{username}', '{password}')"""
            self.mysql.query.execute(query)
            self.mysql.db_connection.commit()
            ...
            self.mysql.db_connection.commit()
            self.errors.set("Registration Successful, wait for admin approval.")
            self.submit_button["state"] = "disabled"
            self.error_label["style"] = "SuccessfulLoginRegisterLabel.TLabel"
            after_timer = threading.Timer(5, self.reset_field_data)
            after_timer.start()
        except db.Error as error:
            if(error.msg == f"Duplicate entry '{self.email_value.get()}' for key 'email'"):
                self.errors.set("E-Mail already registerd with another user.")
            elif(error.msg == f"Duplicate entry '{self.username_value.get()}' for key 'username'"):
